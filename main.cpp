...
void run_loop(){
    ...
    while(!LIST_OF_EVENTS.empty()) {
		...
		if(Transaction::get_number_of_transactions() > MAX_TRANSACTIONS) break;
...
void log_data(){
	FILE* f = fopen("block_info.log.csv", "w");
...
int parse_opt(int key, char *arg, struct argp_state *state) {
    ...
    switch (key) {
        ...
        case 'o': 
			...
			break;
		case 't':
			MEAN_TRANSACTION_INTER_ARRIVAL_TIME = atof(arg);
			break;
		case 'a':
			INITIAL_AMOUNT = atoi(arg);
			break;
		case 'k':
			AVG_INTERARRIVAL_BLOCK_TIME = atoi(arg);
			break;
		case 's':
			rng.set_seed(atoi(arg));
			break;
		case 'b':
			MAX_BLOCKS = atoi(arg);
			break;
		case 'x':
			MAX_TRANSACTIONS = atoi(arg);
			break;
        default:
...
void argparsing(int argc, char * argv[]){
	static struct argp_option options[] = {
		{"z0", 'z', 0, 0, "fraction of nodes are slow (example: --z0 0.4)"},
		{"z1", 'o', 0, 0, "fraction of nodes having low cpu (example: --z1 0.4)"},
		{"ttx", 't', 0, 0 , "Mean time interval between transaction generated by each nodes"},
		{"ia", 'a', 0, 0, "Initial ammout for each peers"},
		{"tk", 'k', 0, 0, "Mean inter arrival block time"},
		{"seed", 's', 0, 0, "Seed of random number generator class"},
		{"blk", 'b', 0, 0, "Maximum number of blocks (stopping criteria)"},
		{"txn", 'x', 0, 0, "Maximum number of transaction (stopping criteria)"},
		{"file", 'f', 0, 0, "file name generated by graph.py"},
		{0}
	};
	char arg_doc[] = "";
	const char doc[] = "this is a program to simulate blockchain";
	struct argp args = {options, parse_opt, arg_doc, doc};
